...
#define UIColorFromRGBWithAlpha(rgbValue,a) [UIColor  colorWithRed:((float)((rgbValue & 0xFF0000) >> 16))/255.0  green:((float)((rgbValue & 0xFF00) >> 8))/255.0  blue:((float)(rgbValue & 0xFF))/255.0 alpha:a] //RGB转UIColor（带alpha值）
...
- (void)layoutSubviews {
    ...
    long subviewsCount      = containtViews.count;
    _scrollView.frame       = CGRectInset(self.bounds, 0, 0);
    _scrollView.contentSize = CGSizeMake(CGRectGetWidth(_scrollView.frame) * subviewsCount, CGRectGetHeight(_scrollView.frame));
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    long index = _scrollView.contentOffset.x;
